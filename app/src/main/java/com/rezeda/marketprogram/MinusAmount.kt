...
import com.rezeda.marketprogram.adapters.MinusItemAdapter
...
class MinusAmount : AppCompatActivity() {

    companion object{
        var total_price_minus = 0.0f
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_minus_amount)

        var product_name = findViewById<TextView>(R.id.product_name_minus)
        var product_price: String? = null
        var edit_amount = findViewById<EditText>(R.id.amount_minus)
        var cartList = JSONHelper.importFromJSON(this)
        var sale_list = findViewById<RecyclerView>(R.id.minus_list)
        var btn_add_product = findViewById<Button>(R.id.btn_add_product)
        var add_amount = findViewById<Button>(R.id.btn_minus_amount)
        var btn_minus = findViewById<Button>(R.id.btn_minus)
         total = findViewById<TextView>(R.id.total_minus)
        var back_from_sale = findViewById<ImageView>(R.id.back_from_minus)
        ...
        if(cartList!=null){
            ...
            var saleItemAdapter = MinusItemAdapter(cartList, this)
            sale_list.adapter = saleItemAdapter
            for(el in cartList){
                total_price_minus += el.summa
                sale_info += "\n${el.sale_name}, ${el.amount}, ${el.summa}"
            }
            total.text = total_price_minus.toString()
        }
        ...
        add_amount.setOnClickListener {
            ...
            if (product_name.text.isEmpty()){
                ...
                var saleItemAdapter = MinusItemAdapter(saleList, this)
                sale_list.adapter = saleItemAdapter
        ...
        btn_add_product.setOnClickListener {
            startActivity(
                Intent(
                    this,
                    MinusSearchActivity::class.java
                )
            )
        ...
        btn_minus.setOnClickListener {
            if(cartList==null){
                ...
                cartList!!.clear()
                JSONHelper.exportToJSON(this, cartList!!)
                Toast.makeText(this, "Списано", Toast.LENGTH_LONG).show()
                var saleList = cartList
